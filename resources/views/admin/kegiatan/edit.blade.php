...
<div class="page-wrapper">
    ...
    <div class="page-body">
        <div class="container-xl">
            <div class="row">
                <div class="col-md-8">
                    <form method="POST" action="{{ route('admin.kegiatan.update', $kegiatan->id) }}" class="card">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="lokasi" class="form-control @error('lokasi') is-invalid @enderror" value="{{ old('lokasi', $kegiatan->lokasi) }}">
                                @error('lokasi')<span class="invalid-feedback">{{ $message }}</span>@enderror
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Tanggal <span class="text-danger">*</span></label>
                                <input type="date" name="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', $kegiatan->tanggal) }}" required>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="penanggung_jawab" class="form-select @error('penanggung_jawab') is-invalid @enderror" required>
                                    ...
                                    @foreach($penanggungJawab as $pj)
                                        <option value="{{ $pj->id }}" {{ old('penanggung_jawab', $kegiatan->penanggung_jawab) == $pj->id ? 'selected' : '' }}>{{ $pj->nama }}</option>
                                    @endforeach
                                </select>
